...
export default function OrdersPage() {
  ...
  // Safely read orders from the Shopify customer object
  const orders = (customer as any)?.orders?.edges ?? [];
      ...
      {orders.length === 0 ? (
        ...
        orders.map(({ node }: any) => (
          <div key={node.id} className="order-row">
            ...
            >
              Placed on {new Date(node.processedAt).toLocaleDateString()} •
              Payment: {node.financialStatus} • Fulfillment:{" "}
